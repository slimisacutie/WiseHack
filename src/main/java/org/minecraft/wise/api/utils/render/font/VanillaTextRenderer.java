...
public class VanillaTextRenderer implements IMinecraft {
    ...
    public void drawWithShadow(MatrixStack matrices, String text, float x, float y, int color) {
        if (FontMod.INSTANCE.shadow.getValue()) {
            draw(matrices, text, x + 1.0f, y + 1.0f, color, true);
        } else {
            draw(matrices, text, x + 0.4f, y + 0.4f, color, true);
        }
    ...
    private void draw(String text, float x, float y, int color, Matrix4f matrix, boolean shadow) {
        ...
        draw(text, x, y, color, shadow, matrix, immediate, TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
        immediate.draw();
    }
    ...
    public static class Drawer implements CharacterVisitor {
        ...
        public boolean accept(int i, Style style, int j) {
            ...
            if (!(glyphRenderer instanceof EmptyGlyphRenderer)) {
                ...
                ((ITextRenderer) mc.textRenderer).hookDrawGlyph(glyphRenderer, bl, style.isItalic(), m, this.x, this.y, this.matrix, vertexConsumer, g, h, l, f, this.light);
            }
        ...
        public void drawLayer() {
            if (this.rectangles != null) {
                GlyphRenderer glyphRenderer = ((ITextRenderer) mc.textRenderer).hookGetFontStorage(Style.DEFAULT_FONT_ID).getRectangleRenderer();
                VertexConsumer vertexConsumer = this.vertexConsumers.getBuffer(glyphRenderer.getLayer(this.layerType));
